...
async def backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    backup_data = {
        ...
        "zone_ids": zone_ids,
        "poison_zones": poison_zones,
        "airdrops": airdrops,
        "dungeon_zones": dungeon_zones
    }

    # Save to file
    filename = f"battleground_backup.json"
    with open(filename, "w") as f:
        json.dump(backup_data, f, indent=4)

    await update.message.reply_text(f" Backup completed! File saved as `{filename}`")
...
async def kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
 ...
 async def hp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid not in players:
        await update.message.reply_text(" You are not in the game yet. Use /join to enter the lobby.")
        return
    ...
    # Heart bar
    heart_bar = "" * hp + "" * (max_hp - hp)
    # Shield bar
    shield_bar = "" * shield + "" * (max_shield - shield)

    text = f" HP Status for {name}:\n"
    text += f"{heart_bar} ({hp}/{max_hp})\n"
    text += f"{shield_bar} ({shield}/{max_shield})"
